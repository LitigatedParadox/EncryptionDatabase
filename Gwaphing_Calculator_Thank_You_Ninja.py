...
def Graph(Scale, Detail, function):
    root = tk.Tk()
    ...
    for iteration in range(1, (total_x + 1)):
        x_position = (-10 * Scale) + ((1/Detail * Scale) * iteration)
        x_location = (((x_position/Scale) + 10)/20) * 1000
        y_position = calculate(x_position, function)
        try:
